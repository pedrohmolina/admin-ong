package com.antares.sirius.service.impl;

import com.antares.commons.service.impl.BaseServiceImpl;
import com.antares.sirius.dao.TipoDocumentoDAO;
import com.antares.sirius.model.TipoDocumento;
import com.antares.sirius.service.TipoDocumentoService;

/**
 * Implementacion de la interfaz TipoDocumentoService.
 *
 * @version 1.0.0 Created 23/11/2010 by Julian Martinez
 * @author <a href:mailto:otakon@gmail.com> Julian Martinez </a>
 *
 */
public class TipoDocumentoServiceImpl extends BaseServiceImpl<TipoDocumento, TipoDocumentoDAO> implements TipoDocumentoService {

}
